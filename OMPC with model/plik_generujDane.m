...
%--------------------------------------------------------------------------
%Stałe parametry
scale = 1; %skala siatki macierzy [1]
...
Uwin = 0.95; %przenikalność cieplna okna [W/m2K] [6]
SC = 1; %współczynnik zacienienia okna <1 - niezacienione> [7]
alpha_wall = 0.3; %współczynnik absorpcji światła przez ściany (zależny od koloru) [8]
alpha_floor = 0.5; %współczynnik absorpcji światła przez podłogę (brązowe panele winylowe) [9]
cw = 4180; % ciepło właściwe wody [10]
dw = 980; % gęstość wodyd [11]
kw = 0.624; % przewodność cieplna wody [12]
u = 0.000547; % lepkość wody [13]
cp = 1005; % ciepło właściwe powietrza [14]
dp = 1.1204; % gęstość powietrza [15]
k_tube = 0.35; %współczynnik przewodzenia rur (PEX) [16]
V_buf = 0.5; %objętość zbiornika buforowego [m^3] [17]
COP = 4; %współczynnik sprawności [18]
area_buf = 2*pi*((V_buf/(4*pi))^(1/3))^2 + 2*pi*((V_buf/(4*pi))^(1/3))*4*(V_buf/(4 * pi))^(1/3); %powierzchnia styku bufor - temperatura na zewnątrz [19]
U = 0.644; %współczynnik przenikania cieplnego zbiornika [20]
alpha0 = 0.4; %punkt pracy zaworu centralnego [21] - reprezentuje spadek przepływu
Kv = 2.5; %współczynnik przepustowości zaworu
coefficients = [scale h1 h2 roofAngle Awin Uwin SC alpha_wall alpha_floor cw dw kw u cp dp k_tube V_buf COP area_buf U alpha0 Kv];
...
floor1Equipment = [0 1 1; % Czy posiada okno
                   0 2 1; % ile okien
                   0.016 0.016 0.016; % średnica rur (zewnętrzna)
                   0.002 0.002 0.002; % grubość ścianki
                   0.2 0.2 0.2; % rozstaw rur
                   0 4 3]; % przepływ wody [l/min]
floor2Equipment = [0 0 1; % Czy posiada okno
                   0 0 1; % ile okien
                   0.016 0.016 0.016; % średnica rur (zewnętrzna)
                   0.002 0.002 0.002; % grubość ścianki
                   0.2 0.2 0.2; % rozstaw rur
                   3 3 4]; % przepływ wody [l/min]
...
roomEquipment = table(floor1Equipment);
...
nc = 24; npred = 48; na = 48;
time = 3600*24; %czas symulacji [s]
w_PumpCost = 15; %współczynnik kosztów cen energii
Q = 125*(C'*C); Q(end,end) = 1e-10;
R = 0.25*eye(size(B,2));

%Trajektorie referencyjne i zakłóceń
trajPrice = [...
    0.45, 0.45, 0.45, 0.45, 0.45, 0.45, ...   % 0:00–5:00 (nocna taryfa)
    0.95, 0.95, 0.95, ...                     % 6:00–8:00 (szczyt poranny)
    0.65, 0.65, 0.65, 0.65, 0.65, 0.65, ...   % 9:00–14:00 (standardowa taryfa)
    0.65, 0.65, ...                           % 15:00–16:00 (standardowa taryfa)
    1.00, 1.00, 1.00, 1.00, ...               % 17:00–20:00 (szczyt wieczorny)
    0.65, 0.55, 0.45 ...                      % 21:00–23:00 (spadek i nocna taryfa)
]; 
...
T_buf = 49.5; T_bufMAX = 55; T_bufMIN = 45;
P_MAX = 10000; P_MIN = 0;
x0 = [x0;T_buf];
xmax = [xmax; T_bufMAX];
xmin = [xmin; T_bufMIN];
